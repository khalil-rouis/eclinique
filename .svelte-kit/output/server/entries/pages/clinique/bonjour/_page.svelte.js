import { t as attr_class, v as attr, y as escape_html } from "../../../../chunks/server.js";
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (tab == 1) $$renderer.push(`<!--[0--><div class="flex flex-col gap-1"><input id="email"${attr_class(`input w-full min-w-xs outline-none `)} type="email" placeholder="Saisissez votre adresse e-mail"${attr("value", login_information.email)}/></div>`);
		else $$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--> `);
		if (tab == 2) $$renderer.push(`<!--[0--><div class="flex flex-col gap-1"><div class="flex w-full items-center gap-4"><p class="pointer-events-none fieldset-label text-lg select-none">+216</p> <input id="phone"${attr_class(`input outline-none `)} type="text" placeholder="Saisissez votre numéro du telephone"${attr("value", login_information.phone_number)}/></div></div>`);
		else $$renderer.push("<!--[-1-->");
