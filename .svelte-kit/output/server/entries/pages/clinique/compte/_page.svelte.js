...
function AppointmentBoard($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (saveStatus !== "idle") {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<div class="toast toast-end toast-bottom z-50"><div${attr_class("alert", void 0, {
		...
		if (appointments.length === 0) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<p class="w-full py-2 text-center text-sm text-base-content/40 italic">Aucun rendez-vous</p>`);
		} else $$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--></div></div>`);
	});
}
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		if (!data.isVerified) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<img src="../unverified.svg" alt="" class="absolute inset-0 top-0 bottom-0 m-auto h-48 w-48 opacity-40 invert md:h-72 md:w-72"/>`);
		} else {
			$$renderer.push("<!--[-1-->");
